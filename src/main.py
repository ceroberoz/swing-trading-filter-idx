...
Examples:
  ...
  python -m src.main --backtest --detailed --charts           # Full analysis with charts
    ...
    parser.add_argument(
        "--show-lists", action="store_true", help="Show available watchlists"
    )

    # Backtesting arguments
    parser.add_argument(
        "--backtest", action="store_true", help="Run backtesting instead of live scan"
    )
    parser.add_argument(
        "--start-date", dest="start_date", help="Backtest start date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--end-date", dest="end_date", help="Backtest end date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--detailed",
        action="store_true",
...
def run_backtest(args):
    ...
    print(
        f"{Style.BRIGHT}{Fore.CYAN}Starting Swing Trading Backtesting...{Style.RESET_ALL}"
    )

    # Determine tickers to backtest
    if args.tickers:
        tickers_to_test = [
            t if t.endswith(".JK") else f"{t.upper()}.JK" for t in args.tickers
        ]
        ...
        tickers_to_test = config.load_watchlist(
            "idx_liquid"
    ...
    if args.detailed:
        ...
        for ticker, ticker_result in results.get("ticker_results", {}).items():
            print(
                report_generator.generate_detailed_ticker_report(ticker, ticker_result)
            )
            print("-" * 60)
    else:
        # Show summary report
...
def main():
    ...
    print(
        f"{Style.BRIGHT}{Fore.CYAN}Starting Swing Trading Scanner (IDX)...{Style.RESET_ALL}"
    )
    print(f"Mode: {scan_mode}")
    print(f"Strategy: EMA{config.FAST_EMA} / EMA{config.SLOW_EMA} Crossover")
    print(
        f"Target Profit: {config.TARGET_PROFIT_MIN * 100:.0f}-{config.TARGET_PROFIT_MAX * 100:.0f}% | Stop Loss: Dynamic (ATR {config.ATR_MULTIPLIER}x)"
    )
    ...
    print(
        f"Market Regime: {market_ctx['market_regime']} | MTF: {'ON' if config.ENABLE_MTF else 'OFF'}"
    )
    print("-" * 60)
    ...
    print(f"{Fore.CYAN}Step 1/3: Fetching historical data...{Style.RESET_ALL}")
    try:
        dfs = data.fetch_data_batch(tickers_to_scan)
    except Exception as e:
        print(f"{Fore.RED}Batch fetch failed: {e}{Style.RESET_ALL}")
        dfs = None

    if dfs is None:
        print(f"{Fore.RED}Failed to fetch data. Try again later.{Style.RESET_ALL}")
        return
        ...
        if df is not None:
            ...
            if analysis:
                ...
                if analysis["is_setup"] or show_all:
                    ...
                    sr_display = (
                        f"{support:.0f}/{resistance:.0f}" if support > 0 else "-"
                    )
