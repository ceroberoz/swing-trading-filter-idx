...
def analyze_support_resistance(current_price, pivot_data, swing_data):
    ...
    nearest_resistance = (
        min(resistances_above) if resistances_above else max(resistances)
    )
...
def combine_signals(base_analysis, weekly_ctx, market_ctx):
    ...
    if original_signal == "BUY":
        ...
        if config.ENABLE_MTF and config.MTF_REQUIRED_FOR_BUY and not weekly_ok:
            ...
            and config.MARKET_FILTER_MODE == "BLOCK"
            and not risk_on
        ):
...
def analyze_ticker(df, market_ctx=None):
    ...
    if df is None or len(df) < max(
        config.SLOW_EMA, config.MACD_SLOW + config.MACD_SIGNAL, config.ATR_PERIOD
    ):
        return None
    ...
    crossover_today = (last_row["EMA_Fast"] > last_row["EMA_Slow"]) and (
    ...
    if crossover_today:
        ...
        vol_ratio = (
            last_row["Volume"] / last_row["Vol_Avg"] if last_row["Vol_Avg"] > 0 else 0
        )
        ...
        stop_loss, take_profit_min, take_profit_max = get_risk_levels(
    ...
    price_vs_ema_pct = (
        ((current_price - ema_slow_value) / ema_slow_value) * 100
        if ema_slow_value > 0
        else 0
    )
    ...
    if config.ENABLE_MTF or config.ENABLE_MARKET_FILTER:
        ...
        base_analysis["strategy"] = get_investment_strategy(
            base_analysis, weekly_ctx, market_ctx
        )
